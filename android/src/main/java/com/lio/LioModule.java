...
public class LioModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void checkoutOrder(int amount, String paymentCode) {
        if (paymentCode.equals("debito")) {
            Log.d(TAG, paymentCode + " " + amount);
            orderManager.checkoutOrder(order.getId(), amount, PaymentCode.DEBITO_AVISTA, createPaymentListener());
        } else {
